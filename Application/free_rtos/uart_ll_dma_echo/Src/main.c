...
void StartDefaultTask(void const * argument)
{
    ...
    Local_enError = uart_init(UART_CHANNEL_1);
    configASSERT(Local_enError == UART_ERROR_NONE);

    Local_enError = uart_init(UART_CHANNEL_2);
    configASSERT(Local_enError == UART_ERROR_NONE);

    Local_enError = uart_init(UART_CHANNEL_3);
    configASSERT(Local_enError == UART_ERROR_NONE);
    ...
    for(;;)
    {
        ...
        osDelayUntil(&Local_u32LastWakeup, 10);
    }
