 ...
 * @callergraph
 *
 * @param [in, out] psUart  : pointer to UART chanel's UART_t
 *
 * @return void
 *
 * */
...
/**
 * @brief UART RX task, copies received bytes from UART DMA RX buffer into UART RX buffer
 *
 * @param [in] pvUart : pointer to UART_t instance
 *
 * @return void
 *
 * */
static void UART_vidUartRxTask(void const * const pvUart);

/**
 * @brief UART TX task, starts DMA transfer to send bytes from UART TX buffer
 *
 * @param [in] pvUart : pointer to UART_t instance
 *
 * @return void
 *
 * */
static void UART_vidUartTxTask(void const * const pvUart);
...
typedef struct uart_dma_context_t
{
        ...
        osThreadId          rx_task_handle;             /**<  UART RX task handle  */
        osThreadId          tx_task_handle;             /**<  UART TX task handle  */
} UART_DMA_Context_t;
...
typedef struct uart_t {
        ...
        osThreadDef_t * const   rx_thread;      /**<  UART channel's RX thread  */
        osThreadDef_t * const   tx_thread;      /**<  UART channel's TX thread  */
} UART_t;
...
static osThreadDef(UART_Ch_1_RxThread, UART_vidUartRxTask, osPriorityLow, 0, UART_RTOS_RX_TASK_STACK_WORDS);
...
static osThreadDef(UART_Ch_1_TxThread, UART_vidUartTxTask, osPriorityLow, 0, UART_RTOS_TX_TASK_STACK_WORDS);
...
static const UART_t UART_1 = {
        ...
        .rx_thread = (osThreadDef_t*)&os_thread_def_UART_Ch_1_RxThread,
        .tx_thread = (osThreadDef_t*)&os_thread_def_UART_Ch_1_TxThread,
};
...
static osThreadDef(UART_Ch_2_RxThread, UART_vidUartRxTask, osPriorityLow, 0, UART_RTOS_RX_TASK_STACK_WORDS);
...
static osThreadDef(UART_Ch_2_TxThread, UART_vidUartTxTask, osPriorityLow, 0, UART_RTOS_TX_TASK_STACK_WORDS);
...
static const UART_t UART_2 = {
        ...
        .rx_thread = (osThreadDef_t *)&os_thread_def_UART_Ch_2_RxThread,
        .tx_thread = (osThreadDef_t *)&os_thread_def_UART_Ch_2_TxThread,
};
...
static osThreadDef(UART_Ch_3_RxThread, UART_vidUartRxTask, osPriorityLow, 0, UART_RTOS_RX_TASK_STACK_WORDS);
...
static osThreadDef(UART_Ch_3_TxThread, UART_vidUartTxTask, osPriorityLow, 0, UART_RTOS_TX_TASK_STACK_WORDS);
...
static const UART_t UART_3 = {
        ...
        .rx_thread = (osThreadDef_t *)&os_thread_def_UART_Ch_3_RxThread,
        .tx_thread = (osThreadDef_t *)&os_thread_def_UART_Ch_3_TxThread,
};
...
/**
 * Array to UART handles (disabled UART channels are NULL)
 * */
...
static inline void UART_DMA_vidTransmitData(const UART_t * const psUart)
{
    ...
    {
        /*  DMA transfer is ongoing, UART_enWrite() was called before DMA finishes */
        return;
    }
    ...
    /**
     * - When called from UART_enWrite() this condition will always be true
     * - When called from DMA_IRQHandler() this condition will always be true until TX buffer is empty
     * */
...
static void UART_vidUartRxTask(void const * const pvUart)
{
    UART_t * Local_psUart = (UART_t *)pvUart;
    osEvent Local_sEvent;

    while(1)
    {
        /* wait for signal from UART IDLE IRQ or DMA RX channel's HT/TC IRQ */
        Local_sEvent = osSignalWait(0, (uint32_t)-1);
...
static void UART_vidUartTxTask(void const * const pvUart)
{
    ...
    while(1)
    {
        /* wait for signal from DMA TX channel's TC IRQ */
        Local_sEvent = osSignalWait(0x01, (uint32_t)-1);
        ...
        /* transmit more data from UART TX buffer (if it's not empty) */
...
UART_Error_t UART_enWriteBlocking(const UART_Channel_t enChannel, uint8_t const * const pu8Data, uint32_t u32Len)
{
    ...
    uint32_t Local_u32WriteCount = 0;
...
{
    /*  check error flags   */
    if((LL_USART_IsEnabledIT_ERROR(psUart->uart_handle) || LL_USART_IsEnabledIT_PE(psUart->uart_handle)) &&
        (LL_USART_IsActiveFlag_ORE(psUart->uart_handle) || LL_USART_IsActiveFlag_PE(psUart->uart_handle))
    )
    {
    ...
    /*  check IDLE flag  */
    if(LL_USART_IsEnabledIT_IDLE(psUart->uart_handle) && LL_USART_IsActiveFlag_IDLE(psUart->uart_handle))
    {
        LL_USART_ClearFlag_IDLE(psUart->uart_handle);

        /* notify UART channel's RX task */
        osSignalSet(psUart->dma_context->rx_task_handle, 0x01);
    }
}
...
static void UART_DMA_TX_IRQHandler(const UART_t * const psUart)
{
    ...
    if(LL_DMA_IsEnabledIT_TC(psUart->dma_handle, psUart->dma_tx_channel) && psUart->dma_tx_is_active_flag_tc(psUart->dma_handle))
    {
        ...
        /* notify UART channel's TX task */
        osSignalSet(psUart->dma_context->tx_task_handle, 0x01);
    }
}
...
static void UART_DMA_RX_IRQHandler(const UART_t * const psUart)
{
    ...
    if(LL_DMA_IsEnabledIT_TC(psUart->dma_handle, psUart->dma_rx_channel) && psUart->dma_rx_is_active_flag_tc(psUart->dma_handle))
    {
